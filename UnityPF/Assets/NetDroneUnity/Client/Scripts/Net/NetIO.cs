		...
		public class CNetIO {
			...
			Clear() {
				if(isptr(m_kConnector)) {
					if(m_kConnector != g_kNetMgr.GetCurrentConnector()) {
						m_kConnector.Disconnect(m_kConnector.GetSocket());
						m_kConnector = null;
					}
				}
				m_tkDelayPingTick = 0;
				m_tkSendPingHostTick = 0;
			...
			public void			SetSendPingHostTick(tick_t o)	{ m_tkSendPingHostTick = o; }
			public tick_t		GetSendPingHostTick()			{ return m_tkSendPingHostTick; }
			...
			private tick_t		m_tkSendPingHostTick = 0;
